...
export async function createInvoicePdf(sale: Sale) {
  ...
  autoTable(doc, {
    ...
    theme: 'grid',
    margin: { left: 36, right: 36 },
    head: [['Sl. No.', 'Description of Goods', 'Quantity', 'MRP', 'Disc %', 'Rate', 'Amount']],
    ...
    foot: [['', 'Total', `${sale.items.reduce((sum, item) => sum + item.quantity, 0).toLocaleString('en-IN')} Pcs`, money(sale.subtotal), money(sale.discount), '', money(sale.grandTotal)]],
